...
class Game:
    ...
    def __init__(self, players):
        self.players = {id:{"name": User(players[id]["name"])} for id in players}
        self.board = Board()

    def initPlayers(self):  # beginning of the game only
    ...
    def orderOfPlayers(self, L):
        """After initPlayers(), we choose randomly what is the order of play"""

        input(
            "Bienvenue a tous dans cette super partie de Monopoly, nous avons tout d'abord tirer au sort l'ordre de jeu des differents joueurs")
        random.shuffle(L)
        n = len(L)
        for i in range(n):
    ...
    def playerHasAllColorStreets(self, player, positionOfStreet):
        """
        ...
        """
        streetColor = self.board.getBox(positionOfStreet).getColor()
    ...
    def turn(self, order):
        ...
        3- if the player has not money anymore --> he looses
        """
